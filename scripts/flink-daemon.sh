...
CORES=$3
echo "${@:5}"
if [ "$DAEMON" = "taskexecutor" ]; then
   ARGS=("${@:5}") # get remaining arguments as array
else
    ARGS=("${@:3}")
fi
bin=`dirname "$0"`
...
case $STARTSTOP in
    ...
    (start)
        ...
        if [ "$DAEMON" = "taskexecutor" ]; then
            echo $CORES
            taskset -c $CORES $JAVA_RUN $JVM_ARGS ${FLINK_ENV_JAVA_OPTS} "${log_setting[@]}" -classpath "`manglePathList "$FLINK_TM_CLASSPATH:$INTERNAL_HADOOP_CLASSPATHS"`" ${CLASS_TO_RUN} "${ARGS[@]}" > "$out" 200<&- 2>&1 < /dev/null &
        else
